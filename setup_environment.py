...
def install_dependencies():
    ...
    run_command(f"{pip_cmd} install torch==1.12.1+cu116 torchvision==0.13.1+cu116 --extra-index-url https://download.pytorch.org/whl/cu116")
    ...
    run_command(f"{pip_cmd} install -v -U git+https://github.com/facebookresearch/xformers.git@v0.0.18#egg=xformers")
    
    # Install mmcv
    print("Installing mmcv...")
    run_command(f"{pip_cmd} install mmcv==1.7.1 openmim")
    run_command(f"./venv/bin/mim install mmcv-full" if os.name != 'nt' else "venv\\Scripts\\mim install mmcv-full")
